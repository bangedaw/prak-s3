...
depth pohon 
  | isOneElmt pohon = 1
  | otherwise = 
    if isExistLeft pohon && isExistRight pohon then
      if depth (left pohon) >= depth (right pohon) then
        1 + depth (left pohon)
      else 
        1 + depth (right pohon)
    else if isExistLeft pohon then 1 + depth (left pohon)
    else if isExistRight pohon then 1 + depth (right pohon)
    else if isTreeEmpty pohon then 0
    else 1
